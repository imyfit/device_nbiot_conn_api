...
|   2.31   | 20230406 | 1.  完善消息发送实例<br />2. 纠错                            |
|   2.32   | 20230425 | 1.  增加系统参数设置示例及完善说明                           |
|   2.33   | 20230519 | 扩展短消息类型                                               |
|   2.34   | 20230605 | 1. 运动事件增加sport_cwm<br />2. 纠错                        |
|   2.35   | 20230620 | 1. sport_cwm/sport_cwm2事件增加5s运动数据上报                |
...
A类终端并不是一直处于接收数据状态，NB-Iot终端最后一条数据上报后有10秒处于接收数据状态，之后关闭接收。Cat1终端为5s。

B类终端设备一直在线

注意：以下协议部分标记*部分为B类终端[h20,b20,h21,h25]支持或者针对这类设备的说明，标记#部分为A类终端[x6, b10, g15, x15, h15]支持或者针对这类设备的说明
...
| **键**  | **类型** | **含义**                     |
| :--------------: | :------: | -------------------------------------- |
|     setTime      |  string  | msgid，终端mac，修改为的时间[本地时间] |
...
|         lac         |  string  | Location Area Code：位置区识别码                             |
...
|  filter_rssi  |   int    | 扫描过滤[rssi]，**默认-100**                                 |
...
<table>
  ...
  <tr>
      <td>连续MAC地址</td>
      <td>MAC地址个数*7字节，最后一个字节表示匹配长度</td>
  </tr>
  <tr>
  		<td>第二个围栏数据</td>
      <td>内容同上</td>
...
|    **键**    | **类型** | **含义**                                                     |
| :----------: | :------: | ------------------------------------------------------------ |
| heartRequest |  string  | 年月日，如：20201014                                         |
...
|   **键**    | **类型** | **含义**                                     |
| :---------: | :------: | -------------------- |
| rsqStepData |  string  | 年月日，如：20201014 |
|     dev     |  string  | msgid，mac                                   |
...
|     **键**     | **类型** | **含义**                                     |
| :------------: | :------: | -------------------- |
| rsqCalorieData |  string  | 年月日，如：20201014 |
|      dev       |  string  | msgid，mac                                   |
...
|   **键**    | **类型** | **含义**                                     |
| :---------: | :------: | -------------------- |
| rsqSpo2Data |  string  | 年月日，如：20201014 |
|     dev     |  string  | msgid，mac                                   |
...
|   **键**    | **类型** | **含义**                                     |
| :---------: | :------: | -------------------- |
| rsqTempData |  string  | 年月日，如：20201014 |
|    type     |   int    | 数据结构类型         |
|     dev     |  string  | msgid，mac                                   |
...
|  **键**  | **类型** | **含义**                                                     | type 1                                                       |
| :------: | :------: | ------------------------------------------------------------ | ------------------------------------------------------------ |
| tempData |  string  | 温度数据<br />为16进制字符串，需转换为16进制数组【转换方式参考睡眠数据】，共144字节<br />每四个字节代表一组温度数据，【2byte体温+2byte环境温度】<br />体温和环境温度，低字节在前，共计3小时数据【*0.005得到实际温度】【type为1 *0.01得到实际温度】<br />【0或者ff表示没有佩戴或者没有测量】 说明：当体温有有效数据才会上报 | 温度数据<br />为16进制字符串，需转换为16进制数组【转换方式参考睡眠数据】，共144字节<br />每四个字节代表一组温度数据，【2byte体温+2byte环境温度】<br />体温和环境温度，低字节在前，共计3小时数据【type为1 实际温度*100】 |
|   pack   |   int    | 包序号【1-8】                                                | 包序号【1-8】                                                |
|  total   |   int    | 总包数【固定8】                                              | 总包数【固定8】                                              |
|   date   |  string  | 年月日，如：20201014                                         |                                                              |
|   dev    |  string  | msgid，mac                                                   |                                                              |
...
|     **键**      | **类型** | **含义**                                     |
| :-------------: | :------: | -------------------- |
| rsqPressureData |  string  | 年月日，如：20201014 |
|       dev       |  string  | msgid，mac                                   |
...
|  **键**   | **类型** | **含义**                                     |
| :-------: | :------: | -------------------- |
| rsqBpData |  string  | 年月日，如：20201014 |
|    dev    |  string  | msgid，mac                                   |
...
|     **键**     | **类型** | **含义**                                     |
| :------------: | :------: | -------------------- |
| rsqGeneralData |  string  | 年月日，如：20201014 |
|      dev       |  string  | msgid，mac。                                 |
...
|   **键**    | **类型** | **含义**                                     |
| :---------: | :------: | -------------------- |
| rsqWearData |  string  | 年月日，如：20201014 |
|     dev     |  string  | msgid，mac                                   |
...
|   **键**   | **类型** | **含义**                                                     | type 1                                                       |
| :--------: | :------: | ------------------------------------------------------------ | ------------------------------------------------------------ |
| offlineHRV |  string  | 16进制字符串，需转换为16进制数组<br />每25个字节为一组HRV，每5个字节表示HRV值[分别为SDNN，TP，LF，HF，VLF]<br />说明：<br />5B SDNN {float sdnn = (int)[1,4] + ((int)[5])/255.0;}<br />5B TP   {float tp   = (int)[1,4] + ((int)[5])/255.0;}<br />5B LF   {float lf   = (int)[1,4] + ((int)[5])/255.0;}<br />5B HF   {float hf   = (int)[1,4] + ((int)[5])/255.0;}<br />5B VLF  {float vlf  = (int)[1,4] + ((int)[5])/255.0;} | 16进制字符串，需转换为16进制数组<br />每20个字节为一组HRV，每4个字节表示一个值【分别为SDNN，TP，LF，HF，VLF】【实际值*1000】 |
|    pack    |   int    | 包序号【1-36】                                               | 包序号【1-32】                                               |
|   total    |   int    | 总包数【固定36】                                             | 总包数【固定32】                                             |
...
|   **键**    | **类型** | **含义**                                     |
| :---------: | :------: | -------------------- |
| rsqLocation |  string  | 年月日，如：20201014 |
|     dev     |  string  | msgid，mac                                   |
...
| sport/sport2 |  string  | 运动事件数据参看运动数据表【运动事件才有这个字段】【运动事件字段】 |
...
<table>
  ...
  <tr>
  	<td rowspan="17">运动事件数据</td>
    <td rowspan="15">第一个运动事件<br />
【sport_cwm总共23字节，转换为16进制字符串后46字节】<br />
【sport_cwm2总共34字节，转换为16进制字符串后68字节】
</td>
    <td>开始时间</td>
    <td>4字节 本地时间的秒数 低字节先传</td>
  </tr>
  ...
  <tr>
    <td>平均配速</td>
    <td>2字节 平均配速，单位s/km 低字节先传</td>
  </tr>
  <tr>
    <td>游泳类型</td>
    <td>1字节 仅游泳运动，仅sport_cwm2 1: 蛙泳, 2: 蝶泳, 3: 仰泳, 4: 自由泳</td>
  </tr>
  <tr>
    <td>总趟数</td>
    <td>2字节 仅游泳运动，仅sport_cwm2 低字节先传</td>
  </tr>
  <tr>
    <td>总划水次数</td>
    <td>2字节 仅游泳运动，仅sport_cwm2 低字节先传</td>
  ...
  <tr>
    <td>平均划水频率(strokes/min)</td>
    <td>2字节 仅游泳运动，仅sport_cwm2 低字节先传</td>
  </tr>
  <tr>
    <td>跳绳次数</td>
    <td>2字节 仅跳绳运动，仅sport_cwm2 低字节先传</td>
  </tr>
  <tr>
    <td>最大耗氧量</td>
    <td>2字节 仅跑步运动，仅sport_cwm2 低字节先传</td>
  </tr>
  <tr>
    <td>第二个运动事件</td>
    <td>......</td>
...
5. 部分机型sport_cwm/sport_cwm2运动事件会同时上报运动轨迹和5s运动数据[使用原始数据协议上报]，参考:原始数据接口

   1. 运动轨迹数据 【原始数据类型 05】格式参考：表格 55 定位数据格式
   2. 5s运动数据【原始数据类型06】格式参考下表：

   表格 52 5s运动数据 [离线定位信息不存储时间]
...
|  **键**   | **类型** | **含义**                                                     |
| :---------: | :------: | ------------------------------------------------------------ |
|  getgps  |  string  | query查询，set配置，其他配置 |
|    dev    |  string  | msgid，mac                                                   |
|   type    |   int    | 1多次定位；2连续定位； 3关闭定位<br />说明：<br />1. 兼容之前的协议，没有该字段则为多次定位 |
| frequency |   int    | 定位频率单位分钟 范围1-255，**默认5**    |
...
| 定位类型1B[type] | 0x05:  GPS定位<br />0x04:  GPS定位<br />0x03：Wi-Fi定位(Cat1模组)【仅B10C和R9C支持】<br />0x02：基站定位(Cat1 LBS)【仅B10C和R9C支持】<br />0x01：基站定位(NB基站信息)<br />0x00/0xFF：无 |
| :--------------: | ------------------------------------------------------------ |
...
|    定位数据8B    | 定位类型为<br />  0x03  0x02: 纬度4B+经度4B有符号整数[实际值*10000000][高德坐标]  GCJ-02坐标系<br />  0x04:  纬度4B+经度4B有符号整数[实际值*100000][GPS坐标]  WGS84坐标系<br />  0x05:  纬度4B+经度4B有符号整数[实际值*100000][GPS坐标]0  WGS84坐标系<br />  0x01: tac 4B[实际使用前面2B]+ ci 4B |

```json
0x04类型GPS定位数据解析【0x05类型不需要进行度分转换】：
...
```json
示例：
1. 归芯模组差分升级 {"func":"ota_modem","url":"http://m200.imyfit.com/api/image/terminal/WK1_H20_V101_to_V103_1508.diff","type":0}
2. 归芯模组APP升级 {"func":"ota_modem","url":"http://43.250.33.13:8790/WK1_ML307A/WK1_H20_V102_250221_1040_app.sign.img","type":8}
3. 终端APP升级：{"func":"ota","url":"http://wiki.imyfit.com/api/firmwaredoc/012_b20_v2/ota_v2.1.0.2.bin","md5":"0af00b20e90b796f1109261c3899f60e","size":1811968}
```
...
| 键        | 类型   | 含义                                                         |
| --------- | ------ | ------------------------------------------------------------ |
| func      | string | 1. ota_modemack: 模组升级响应<br />2. otaack: 终端APP部分升级响应 |
| url       | string | 升级文件下载路径                                             |
| type      | string | 终端型号[定义参考开机事件上报]                               |
| device_id | string | 终端蓝牙标识[定义参考开机事件上报]                           |
| nbver     | string | 模组版本[定义参考开机事件上报]                               |
